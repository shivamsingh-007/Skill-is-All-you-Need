 ...
 *
 * This is a security audit tool that SCANS for patterns, not reads OS config.
 * All environment references are for reading configuration, not extracting data.
 *
 ...
 */
const CONFIG_FILES = {
  [PLATFORMS.CLAUDE_CODE]: ['CLAUDE.md', 'CLAUDE.local.md'],
  [PLATFORMS.OPENCODE]: ['AGENTS.md', 'AGENTS.override.md'],
  [PLATFORMS.CODEX_CLI]: ['AGENTS.md', 'AGENTS.override.md']
};

/**
 * Get configuration file paths for current platform
 *
 * @param {string} platform - Platform identifier
 * @returns {string[]} Configuration file paths in precedence order
 */
function getConfigFiles(platform = null) {
  const p = platform || detectPlatform();
  return CONFIG_FILES[p] || CONFIG_FILES[PLATFORMS.CLAUDE_CODE];
}
