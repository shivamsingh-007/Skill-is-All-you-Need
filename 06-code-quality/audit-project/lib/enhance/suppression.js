/**
 * Suppression System for /enhance - Detection Only
 *
 * SECURITY NOTE: This file SCANS for suppression patterns.
...
function applySeverityOverride(finding, config) {
  ...
  const ruleLookup = config.ignore.rules[patternId];
  if (ruleLookup && ruleLookup.level && ruleLookup.level !== 'off') {
    if (['HIGH', 'MEDIUM', 'LOW'].includes(ruleLookup.level.toUpperCase())) {
      const result = ruleLookup.level.toUpperCase();
      return result;
    }
  }
