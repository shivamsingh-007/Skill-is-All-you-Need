/**
 * Project Memory Patterns - Detection Only
 *
 * SECURITY NOTE: This file SCANS for project memory patterns.
...
const projectMemoryPatterns = {
  ...
  deep_nesting: {
    ...
    check: (content) => {
      ...
      // Check list nesting
      const nestedItems = (content.match(/^(?:\s{8,}[-*]|\t{3,}[-*])/gm) || []).length;

      if (h5Plus > 2 || nestedItems > 5 || (h4Plus > 5 && nestedItems > 3)) {
        return {
